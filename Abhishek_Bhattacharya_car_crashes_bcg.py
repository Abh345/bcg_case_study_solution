...
vehicles_in_crashes_df = driver_died_no_airbag_df.join(df_units_use, ["CRASH_ID","UNIT_NBR"])
...
hit_and_run_df = valid_licenses_df.join(df_units_use , ["CRASH_ID","UNIT_NBR"]).filter(col("VEH_HNR_FL") == "Y")
...
extract_numbers_from_damage_level_udf = udf(extract_number,(IntegerType()))
...
damage_level_above_4_df = df_units_use.filter((extract_numbers_from_damage_level_udf("VEH_DMAG_SCL_1_ID") > 4) & (extract_numbers_from_damage_level_udf("VEH_DMAG_SCL_2_ID") > 4))
...
# MAGIC %md
# MAGIC Identify the top 25 states with the highest number of offences

# COMMAND ----------

from pyspark.sql.functions import col, count, desc

# Filter speeding related offences in charges_df
speeding_offences_df = df_charge_use.filter(col("CHARGE").contains("SPEED"))

# Join with primary_person_df to get state information
offences_with_state_df = speeding_offences_df.join(df_primary_person_use, ["CRASH_ID", "UNIT_NBR", "PRSN_NBR"])

# Group by state and count the number of offences
state_offences_df = offences_with_state_df.groupBy("DRVR_LIC_STATE_ID").count().orderBy(desc("count"))

# Get the top 25 states with the highest number of offences
top_25_states_df = state_offences_df.limit(25)
top_25_states = [row["DRVR_LIC_STATE_ID"] for row in top_25_states_df.collect()]

# Display the result
top_25_states_df.show()

# COMMAND ----------

# MAGIC %md
# MAGIC Identify the top 10 most used vehicle colors.

# COMMAND ----------

# Group by VEH_COLOR_ID and count the number of occurrences in units_df
top_vehicle_colors_df = df_units_use.groupBy("VEH_COLOR_ID").count().orderBy(desc("count")).limit(10)
top_vehicle_colors = [row["VEH_COLOR_ID"] for row in top_vehicle_colors_df.collect()]

# Display the result
top_vehicle_colors_df.show()

# COMMAND ----------

# MAGIC %md
# MAGIC Filter data for drivers charged with speeding offences.

# COMMAND ----------

# Filter for drivers charged with speeding offences
drivers_with_speeding_offences_df = speeding_offences_df.select("CRASH_ID", "UNIT_NBR", "PRSN_NBR").distinct()

# Display the result
drivers_with_speeding_offences_df.show()

# COMMAND ----------

# MAGIC %md
# MAGIC Filter for licensed drivers.
...
# MAGIC %md
# MAGIC Combine all these criteria to find the top 5 vehicle makes.

# COMMAND ----------

# Join with licensed drivers
licensed_speeding_drivers_df = drivers_with_speeding_offences_df.join(licensed_drivers_df, ["CRASH_ID", "UNIT_NBR", "PRSN_NBR"])

# Filter for top 10 vehicle colors
licensed_speeding_drivers_with_colors_df = licensed_speeding_drivers_df.join(df_units_use, ["CRASH_ID", "UNIT_NBR"]).filter(col("VEH_COLOR_ID").isin(top_vehicle_colors))

# Filter for top 25 states
final_filtered_df = licensed_speeding_drivers_with_colors_df.filter(col("DRVR_LIC_STATE_ID").isin(top_25_states))

# Group by vehicle makes and count the number of occurrences
top_vehicle_makes_df = final_filtered_df.groupBy("VEH_MAKE_ID").count().orderBy(desc("count")).limit(5)

# Display the result
top_vehicle_makes_df.show()
